// Prometheus text exposition for routy (P4).
//
...
export function renderMetrics(repos, { version, inflight = 0, startedAt = Date.now(), windowMs = 0 } = {}) {
  ...
  out.push(family("routy_requests_total", "counter", "Completed requests by terminal status.",
    byStatus.map((r) => ({ labels: labels({ status: r.status }), value: r.n }))));

  const byNode = repos.stats.requestsByNode({ since });
  out.push(family("routy_requests_by_node_total", "counter", "Completed requests by upstream node and status.",
    byNode.map((r) => ({ labels: labels({ node: r.node, status: r.status }), value: r.n }))));

  const totals = repos.stats.totals({ since });
  out.push(family("routy_tokens_total", "counter", "Tokens observed, by direction.", [
    { labels: labels({ type: "prompt" }), value: totals.promptTokens },
    { labels: labels({ type: "completion" }), value: totals.completionTokens },
  ]));
  out.push(family("routy_cost_usd_total", "counter", "Reported upstream cost in USD.",
    [{ labels: "", value: Number(totals.costUsd) || 0 }]));

  const ttft = repos.stats.ttftByNode({ since });
  out.push(family("routy_ttft_ms_count", "counter", "Requests with a measured time-to-first-token, by node.",
    ttft.map((r) => ({ labels: labels({ node: r.node }), value: r.n }))));
  out.push(family("routy_ttft_ms_sum", "counter", "Sum of measured time-to-first-token, in ms.",
    ttft.map((r) => ({ labels: labels({ node: r.node }), value: r.sum }))));
  out.push(family("routy_ttft_ms_min", "gauge", "Minimum measured time-to-first-token, in ms.",
    ttft.map((r) => ({ labels: labels({ node: r.node }), value: r.min }))));
  out.push(family("routy_ttft_ms_max", "gauge", "Maximum measured time-to-first-token, in ms.",
  ...
  out.push(family("routy_nodes", "gauge", "Configured upstream nodes by status.",
    [...statusCounts].map(([status, value]) => ({ labels: labels({ status }), value }))));

  const now = Date.now();
  const breakers = repos.breakers.all();
  out.push(family("routy_breaker_failures", "gauge", "Consecutive failures recorded per breaker.",
    breakers.map((b) => ({ labels: labels({ scope: b.scope }), value: b.failures }))));
  out.push(family("routy_breaker_open", "gauge", "1 while the breaker's cooldown is in effect.",
    breakers.map((b) => ({ labels: labels({ scope: b.scope }), value: b.state === "open" && b.openUntil && Date.parse(b.openUntil) > now ? 1 : 0 }))));

  out.push(family("routy_inflight_requests", "gauge", "Requests currently being served.", [{ labels: "", value: inflight }]));
  out.push(family("routy_upstream_pools", "gauge", "Open upstream connection pools (one per origin).", [{ labels: "", value: poolOrigins().length }]));
  out.push(family("routy_uptime_seconds", "gauge", "Seconds since the gateway booted.", [{ labels: "", value: Math.round((now - startedAt) / 1000) }]));

  const mem = process.memoryUsage();
  out.push(family("routy_process_resident_memory_bytes", "gauge", "Resident set size.", [{ labels: "", value: mem.rss }]));
  out.push(family("routy_process_heap_used_bytes", "gauge", "V8 heap in use.", [{ labels: "", value: mem.heapUsed }]));
  out.push(family("routy_process_heap_total_bytes", "gauge", "V8 heap reserved.", [{ labels: "", value: mem.heapTotal }]));
  out.push(family("routy_process_external_bytes", "gauge", "Off-heap memory (buffers, streams).", [{ labels: "", value: mem.external }]));

  out.push(family("routy_build_info", "gauge", "Build metadata; always 1.", [{ labels: labels({ version, name: "routy-core" }), value: 1 }]));
