...
//
// Boots dist/routy.mjs as a real gateway (fresh ROUTY_HOME, its own port), drives a
...
//
// Usage: node scripts/smoke.mjs [--bundle dist/routy.mjs]
...
const bundle = path.resolve(root, outFlag >= 0 ? argv[outFlag + 1] : "dist/routy.mjs");
...
try {
  ...
  check("created its own state dir", fs.existsSync(path.join(HOME, "data", "routy.db")));

  const health = await req("GET", "/api/health");
  check("health reports ok", health.status === 200 && JSON.parse(health.body).status === "ok");

  const metrics = await req("GET", "/metrics");
  check("metrics served", metrics.status === 200 && metrics.body.includes("routy_build_info"));
