<#
.SYNOPSIS
  Register / remove / inspect the RE-E gateway as a Windows scheduled task.
...
.EXAMPLE
  powershell -ExecutionPolicy Bypass -File scripts\re-e-task.ps1 -Action install -AtStartup -Port 8010
.EXAMPLE
...
param(
  ...
  [string]$Home = "$env:USERPROFILE\.re-e",
  [int]$Port = 8010,
...
$TaskName = "RE-E Gateway"
$Launcher = Join-Path $PSScriptRoot "re-e-serve.cmd"
...
switch ($Action) {
  "install" {
    ...
    $action = New-ScheduledTaskAction -Execute $Launcher -Argument "`"$RepoRoot`" `"$Home`" $Port" -WorkingDirectory (Join-Path $RepoRoot "re-e-core")
    ...
    Register-ScheduledTask -TaskName $TaskName -Action $action -Trigger $trigger -Settings $settings -Principal $principal `
      -Description "RE-E AI gateway (re-engineering of 9Router). Restart-on-failure; graceful stop via $BaseUrl/api/gateway/shutdown." | Out-Null
    Write-Host "Installed scheduled task '$TaskName' ($(if ($AtStartup) { 'at startup, as SYSTEM' } else { "at logon for $env:USERNAME" }))."
    Write-Host "Start it with:  powershell -File scripts\re-e-task.ps1 -Action start"
  }
