...
//
// Usage: node scripts/build.mjs [--minify] [--out dist/re-e.mjs]
...
const outfile = path.resolve(root, outFlag >= 0 ? argv[outFlag + 1] : "dist/re-e.mjs");

fs.mkdirSync(path.dirname(outfile), { recursive: true });

const result = await build({
  entryPoints: [path.join(root, "bin", "re-e.mjs")],
  outfile,
  bundle: true,
  platform: "node",
  target: "node22",
...
const uiSrc = path.join(repo, "re-e-ui", "dist");
...
if (fs.existsSync(path.join(uiSrc, "index.html"))) {
  ...
  console.log(`  ui      : copied re-e-ui/dist -> ${path.relative(repo, uiDest)}`);
} else {
  console.log("  ui      : re-e-ui/dist not built — bundle serves no dashboard (set RE_E_UI_DIR)");
}
