...
describe("metrics exposition", () => {
  ...
  it("counts requests, tokens and TTFT from usage events", () => {
    ...
    expect(sample(p, "re_e_requests_total", { status: "ok" })).toBe(2);
    expect(sample(p, "re_e_requests_total", { status: "error" })).toBe(1);
    expect(sample(p, "re_e_requests_by_node_total", { node: "a", status: "ok" })).toBe(2);
    expect(sample(p, "re_e_tokens_total", { type: "prompt" })).toBe(16);
    expect(sample(p, "re_e_tokens_total", { type: "completion" })).toBe(27);
    expect(sample(p, "re_e_ttft_ms_count", { node: "a" })).toBe(2);
    expect(sample(p, "re_e_ttft_ms_sum", { node: "a" })).toBe(100);
    expect(sample(p, "re_e_ttft_ms_min", { node: "a" })).toBe(40);
    expect(sample(p, "re_e_ttft_ms_max", { node: "a" })).toBe(60);
  });
  ...
  it("reports node and breaker state as gauges", () => {
    ...
    expect(sample(p, "re_e_nodes", { status: "down" })).toBe(1);
    expect(sample(p, "re_e_nodes", { status: "disabled" })).toBe(1);
    expect(sample(p, "re_e_nodes", { status: "degraded" })).toBe(1);
    expect(sample(p, "re_e_breaker_open", { scope: `node:${down.id}` })).toBe(1);
    expect(sample(p, "re_e_breaker_failures", { scope: `node:${down.id}` })).toBe(3);
    expect(sample(p, "re_e_inflight_requests")).toBe(2);
    expect(sample(p, "re_e_build_info", { version: "0.1.0" })).toBe(1);
  });
  ...
  it("windowMs narrows the counters to recent events only", () => {
    ...
    expect(sample(parse(renderMetrics(repos, { version: "0" })), "re_e_requests_total", { status: "ok" })).toBe(2);
    expect(sample(parse(renderMetrics(repos, { version: "0", windowMs: 60_000 })), "re_e_requests_total", { status: "ok" })).toBe(1);
  });

  it("always exposes live process gauges", () => {
    const p = parse(renderMetrics(repos, { version: "0.1.0" }));
    expect(sample(p, "re_e_process_resident_memory_bytes")).toBeGreaterThan(0);
    expect(sample(p, "re_e_process_heap_used_bytes")).toBeGreaterThan(0);
    expect(sample(p, "re_e_process_external_bytes")).toBeGreaterThanOrEqual(0);
  });
});
